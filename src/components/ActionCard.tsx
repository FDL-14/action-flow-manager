...
import { MoreHorizontal, UserRound, Building2, Calendar, Check, Clock, AlertTriangle, Edit, Trash, FileText, Eye } from 'lucide-react';
...
const ActionCard = ({ action, onDelete, onMenuClick, isProcessing = false, onView }: ActionCardProps) => {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      await updateActionStatus(action.id, 'concluido');
    } catch (error) {
                ...
                <div className="flex flex-wrap gap-2 mt-1.5 items-center">
                  ...
                  {!isCompleted && daysRemaining !== null && (
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      daysRemaining <= 1 ? 'bg-amber-100 text-amber-800' : 
